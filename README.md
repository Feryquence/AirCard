...
- 批量导出可读取的原始卡面和 Wallet 显示缓存；原始资源编码与扩展名不符时仍原样保存并提示。
...
3. **导出**：点击“导出全部卡面文件”，在复选列表中勾选要导出的缓存和原始资源，再选择目标文件夹。默认全选，支持全选/全不选，至少选择一项。结果保存在 `AirCard_output_<卡片Hash>` 子目录，保留原始文件名；缓存另存为 `Wallet 卡面缓存.png` 或 `.jpg`。只读取勾选项，未读取到的资源正常跳过。
