# 设备读写与限制

## 批量导出

点击导出后先尝试通过 AFC 列举卡片目录的顶层名称。受保护路径返回 `0x08` 或 `0x0A` 时回退至 `manifest.json`；其他连接错误不当作空目录。清单通过现有单文件间接读取流程读取并原样归位，不移动整个卡片目录。合并目录和清单中安全的图片路径、PDF 及 `.urls` 索引，读取索引后显示资源复选列表。目录不可列举时，清单不代表完整的当前目录，尤其可能缺少工具后来添加的文件。无有效清单时仍提供 Wallet 缓存，不猜测一组预设文件已存在。

JSON 解析限 4 MiB、16 层深度、2048 条顶层记录。仅接受图片和 PDF 路径及其 `.urls`，不批量读取 pass.json 或本地化文字等其他资料。路径拒绝绝对路径、路径穿越、Windows 保留名称及大小写冲突；允许安全的相对子目录。选择必须属于当前设备、卡片的资源清单。旧单文件 API 保留预设候选名称，界面批量导出使用发现结果。

`.urls` 中的每项记录文件名、URL、size、sha1。远程索引并不证明本地副本不存在：列表分别提供“尝试读取设备副本”和“Apple 原图 · 联网下载”。设备项默认选择，远程项默认不选。同名两项同时勾选时优先下载 Apple 原图，只保存一个同名文件；只选设备副本不会联网补取。资源索引仅在发现阶段内部读取、解析并归位，不出现在勾选列表，也不保存到导出文件夹。实际图片名称取自索引中的条目，不简单删除 .urls 后缀，以兼容 @2x 等真实文件名。

下载仅接受 HTTPS、443 端口及 `.apple.com` 子域地址，拒绝 URL 用户凭据、片段和重定向，不发送设备标识、卡片文件或 Windows 默认凭据。文件须为 1 字节至 32 MiB，大小和 SHA-1 与索引一致才可保存，不转码。校验失败或下载失败会中断并保留此前完成的导出文件，不把损坏的下载保存成成功结果。校验是与设备索引比较，并非对清单签名的验证；不支持的来源会记录原因并跳过该索引。

原始文件保持原字节和文件名。显示缓存读取 `.cache/FrontFace`，必要时尝试 `.pkcache/FrontFace`，从归档的 `faceImage → imageData → NS.data` 提取 PNG/JPEG。缓存可能包含圆角和合成文字，尺寸不等于原始卡面。

批量导出的原始资源若文件头未通过扩展名对应的 PNG/PDF 校验，仍按原文件名、原始字节保存，并在日志和完成提示中说明；不转码、不改名，也不据此判断实际格式或文件是否损坏。此规则只用于原始文件导出，导入、卡面格式识别和显示缓存解码仍保留格式校验。同步、读取、归位和本地保存错误仍会中断导出。

未读取到的资源正常跳过；全部未读到时显示无可导出文件。暂存文件查询遇到 AFC `0x04` 时，仅在成功列举 Media 根目录、确认该暂存文件没有出现后按未收到处理。文件存在但无法读取、连接中断、无法确认归位或本地保存失败，仍会停止。未读取到不能证明受保护目录里的原文件不存在。

重复导出覆盖本次读取到的同名本地文件，保留未读取到的旧文件和其他文件。

导出与应用支持协作式取消：界面立即标记取消，资源边界检查令牌，停止后续读取或写入。本次已开始的同步事务不强行中断，也不放弃正在使用的原生句柄；原文件归位和同步清理不受取消令牌打断。正常完成的当前文件可保存，完成提示报告已保留的文件数量。设备或归位错误不会因同时请求取消而被隐藏。原生 Apple 调用可能阻塞，因此取消不保证即时返回；已写入的卡面不会自动回滚。

## 格式识别与写入

识别在点击应用之后执行。优先检查 `cardBackgroundCombined` 这一组主卡面；没有读到时依次尝试 `background`、`strip`。不会把不同组的格式混合起来，`diffuse` 和显示缓存不能单独用于判断主卡面格式。

同一组资源同时有 PNG 和 PDF 时，需要用户明确选择覆盖格式，再检查导入文件是否需要转换。不能仅凭资源存在判断钱包实际采用哪种格式。无法读取原始资源时取消应用，保留本地预览。

写入集合只包含本次确认的格式：

- PNG：`cardBackgroundCombined@3x.png`、`cardBackgroundCombined@2x.png`。
- PDF：`cardBackgroundCombined.pdf`。

不删除另一格式的已有文件。写入后尝试刷新 `.cache` 和 `.pkcache` 中的 `FrontFace`、`PlaceHolder`、`Preview`。可选缓存刷新不完整会报告警告；同步完成并不保证钱包立即采用新资源。

## 同步与中断

“图书同步”指 Apple 驱动提供的 AirTraffic 文件同步通道。间接读取需要临时将资源移到 Media，通过 AFC 读取，再把本次读取的原始字节通过独立通道放回原位置。归位使用新的资源标识，确认后才清理暂存文件。不能仅凭同步通知或旧暂存文件消失就判定成功。

程序保存并还原三个配置文件：`Books/Books.plist`、`Books/Sync/Books.plist`、`Books/Sync/Upload.plist`。

不快照、覆盖或删除同步服务持有的 SQLite、WAL、SHM 文件，历史记录中的数据库快照也不会回写。服务生成的非空数据库目录保留。

操作记录和读取备份位于 `%LOCALAPPDATA%\AirCard.NetFramework\Recovery`。每次界面操作使用独立的引擎实例，只在内存中跟踪本次新建记录；重试或重启不读取、接续历史未完成操作，也不自动删除其记录或备份。本次批量操作中的延迟文件仍会检查并归位。操作结束后不再自动检查遗留暂存文件；若中断时有文件未归位，重启不会替它归位。设备暂存原文件不可用时保留记录并停止，不自动拿历史备份覆盖当前卡面。

缓存解码发生在原文件归位之后。移动确认不等于对受保护目录进行独立字节回读；此机制仍有兼容性与中断风险。

## 同步失败诊断

日志记录设备型号、iOS 版本、设备会话连接方式及 Apple 驱动版本。AirTraffic 实际通道由 Apple 驱动选择。同步握手依次等待 `SyncAllowed`、`ReadyForSync`、`AssetManifest`，这些步骤完成后才发送资源移动确认。

同步前检查机器级 CoreFP/iTunes 注册路径、iTunes.exe 的 App Paths 注册路径和桌面版 iTunes 默认目录，按顺序加载有效的 64 位 `CoreFP.dll`，并记录路径和版本。使用绝对路径和组件目录解析依赖，不修改注册表、系统 PATH，也不随程序分发 Apple DLL。刷新设备时检查驱动和 CoreFP；无法加载时提示安装或修复完整桌面版 iTunes，并在卡面读写前停止。环境弹窗只提供完整 iTunes 下载与取消。查找失败只能证明这些位置没有可加载的组件，不能据此断定用户未安装任何版本的 iTunes。

Grappa 初始化由 Apple 的 `ATHostConnectionSendSyncRequest` 内部执行；应用不自行生成认证数据或绕过认证。发送请求后通过导出的 `ATHostConnectionGetGrappaSessionId` 记录本机会话是否创建；非零仅表示本机创建成功，仍须等待手机确认。手机日志中的 `Grappa session could not be established` 可定位到该阶段，但不能单凭 `ErrorCode=4` 推断所有环境的具体原因。预加载用于修正组件查找问题，故障设备仍需实测验证。

收到 `SyncFailed` 或提前收到 `SyncFinished` 时，报告等待阶段及设备提供的标量错误字段；不输出整个同步清单。等待 `ReadyForSync` 时被拒绝不代表卡面文件不存在，也不能单凭这一条错误确定是 iOS 或驱动不兼容。缺少文件可以跳过，同步失败仍会中断操作并执行已有的清理检查。排查时请提供版本、连接方式、图书是否已安装并打开，以及失败阶段和错误详情；分享日志前遮盖个人设备和卡片标识。


## 本地同步日志采集

在导出区域勾选“采集导出诊断日志”后，先选择保存文件夹，程序连接所选设备的 `com.apple.syslog_relay`，覆盖资源清单发现与实际导出，并在流程结束后关闭日志连接。未启用日志时，在资源复选列表之后选择文件夹。日志连接启动失败会取消本次导出；采集期间断开会单独报告，不把采集错误当作卡面导出成功。

匹配 `atc` 进程及 AirTraffic、ATFoundation、Books 子系统的实时日志保存为所选文件夹中的 `aircard-device-sync-时间-编号.log`，内容上限 2 MiB。采集默认关闭，不上传内容，不读取历史系统日志。系统隐藏或不输出的细节无法补全；零匹配日志不能证明同步正常。分享前请检查设备和文件标识。普通操作日志仍可通过“保存日志”导出。

## 网络与本地数据

- 不上传卡面，不保存扫描历史。操作日志、恢复记录和原始备份可能包含设备或卡片标识，请勿直接提交到公共仓库。
- USB 设备发现连接本机 Apple 服务；WiFi 模式连接已配对设备。
- PDF 使用本机 Windows 渲染器。
- 用户明确勾选远程原图后，程序访问该卡片 `.urls` 中的 Apple HTTPS 资源地址；远程选项默认不勾选，不上传图片。普通日志不记录完整远程地址，导出文件夹不保存 `.urls` 索引。
- 用户点击“下载完整 iTunes”时打开 Apple 官方下载地址，由用户完成安装。

## 固定上游版本

| 来源 | 版本 |
| --- | --- |
| AirCard-Windows | `b4d07e55fb21cf8be5414c35f5b29614a93135ad` |
| airlift | `c684cd41ca0ded2d1ab780c15f6ead05509ce062` |
| dnSpy | `2b6dcfaf602fb8ca6462b8b6237fdfc0c74ad994` |

主题的原始副本、哈希与许可证保留于 `ThirdParty/dnSpy`。运行和构建不依赖任何开发者的个人磁盘路径。
